...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    while not (expressions.second is nil):
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        target_next = scheme_eval(expressions.second.first, env)
        env.define(target, target_next)
        return expressions.first
        # END Question 5A
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN Question 9A
        target_next = LambdaProcedure(target.second,expressions.second,env)
        env.define(target.first, target_next)
        return target.first
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    target = expressions.first
    if scheme_eval(target,env) is False:
...
def check_formals(formals):
    ...
    """
    if not (formals.second is nil or isinstance(formals.second, Pair)):
        raise SchemeError
    else:
        formal_list = []
        for i in range(len(formals)):
            if not (formals.second is nil or isinstance(formals.second, Pair)) or not scheme_symbolp(formals.first):
                raise SchemeError
            formal_list.append(formals.first)
            formals = formals.second
        if len(set(formal_list))!=len(formal_list):
            raise SchemeError
    # END Question 11B
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Evaluate):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            procedure = scheme_eval(first, env)
            arguments = rest.map(lambda expressions: scheme_eval(expressions, env))
            result = scheme_apply(procedure, arguments, env)
    return result
